...
from ceopardy import VERSION, db
...
class Team(db.Model):
    ...
    def __init__(self, tid, name, handicap=0):
        ...
        app.logger.debug(
            "Team created: name is {}, handicap is {}".format(self.name, self.handicap)
        )
...
class Question(db.Model):
    ...
    def __repr__(self):
        return "<Question {} for {} at col {} row {}>".format(
            self.category, self.score_original, self.col, self.row
        )
...
class Answer(db.Model):
    ...
    def __repr__(self):
        return "<Answer by team id {} to question id {} is {}. Points {}>".format(
            self.team_id, self.question_id, self.response.name, self.score_attributed
        )
